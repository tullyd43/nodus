...
const bootstrap = async () => {
	...
	try {
		if (!gridSystem?.isInitialized || !gridSystem.isInitialized()) {
			...
			console.info(
				`⏱️ Grid initialized in ${gridInitDuration.toFixed(1)}ms`
			);
			try {
	...
	try {
		...
		try {
			...
			updateInputs = policies?.getPolicy?.(
				"ui",
				"bind_bridge_update_inputs"
			);
		} catch {
			/* noop */
		}
		...
		if (bridgeEnabled && stateManager.on) {
			...
			if (typeof maybeUnsub === "function")
				unsubscribeBindBridge = maybeUnsub;
		}
	} catch {
		/* noop */
	}
