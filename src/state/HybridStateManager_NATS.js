...
export class HybridStateManager_NATS {
  ...
  async initializeNATS() {
    ...
    try {
      ...
      console.log('[HybridStateManager_NATS] Connected to NATS:', this.config.natsServers);
  ...
  async publishCommand(command) {
    if (!this.natsClient) {
      console.warn('[HybridStateManager_NATS] NATS not connected. Command queued locally.');
    ...
    try {
      ...
      console.error('[HybridStateManager_NATS] Failed to publish command:', err);
      EventBus.emit('error', err);
    }
  }
  ...
  handleEvent(event) {
    ...
    if (event.entity && event.type === 'entity_updated') {
      ...
      console.debug('[HybridStateManager_NATS] Applying entity update:', event.entity.id);
    }
  }
