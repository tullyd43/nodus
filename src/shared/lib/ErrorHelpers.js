...
export class ErrorHelpers {
	...
	/**
	 * Metrics helper utilities — prefer ActionDispatcher so platform observability
	 * captures metric updates. Fallbacks use the metricsRegistry.
	 * @private
	 */
	#metricKey(name) {
		return String(name || "");
	...
	#incrementMetric(name, value = 1) {
		...
		try {
			this.#metricsRegistry?.increment?.(name, value);
		} catch {
	...
	#setMetric(name, value) {
		...
		try {
			this.#metricsRegistry?.set?.(name, value);
		} catch {
			// swallow
		}
	}
	...
	#recordTimer(name, value) {
		...
		try {
			this.#metricsRegistry?.timer?.(name, value);
		} catch {
			// swallow
		}
	}
	...
	handleError(error, context = {}) {
		...
		if (category) this.#incrementMetric(`errors.by_category.${category}`);
		if (severity) this.#incrementMetric(`errors.by_severity.${severity}`);
		if (component)
			this.#incrementMetric(`errors.by_component.${component}`);
	 ...
	 * @private
	 */
	#recoverPolicyError(_error, _context) {
	...
	setupGlobalHandlers() {
		...
		this.#uiBridge?.addGlobalEventListener("error", (event) => {
			this.handleError(
		...
		}); // V8.0 Parity: Use UIBridge for global event handling.

		// Handle unhandled promise rejections
		this.#uiBridge?.addGlobalEventListener(
			"unhandledrejection",
			(event) => {
		...
		console.warn("[ErrorHelpers] Global error handlers set up");
	}
		...
		try {
			...
			if (result && typeof result.then === "function") {
				return result.finally(() =>
					this.#recordTimer(operation, performance.now() - startTime)
				);
			}

			// Handle sync functions
			const duration = performance.now() - startTime;
			this.#recordTimer(operation, duration);
			return result;
		} catch (error) {
			const duration = performance.now() - startTime;
