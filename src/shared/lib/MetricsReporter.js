 ...
 * @privateFields {#stateManager, #interval, #renderMetrics, #metricsRegistry, #cacheManager, #policiesManager, #errorHelpers, #thresholds, #timer, #isRunning, #reportCount, #fpsUnsubscribe}
 * @privateFields {#stateManager, #interval, #renderMetrics, #metricsRegistry, #cacheManager, #policiesManager, #errorHelpers, #asyncOrchestrator, #uiBridge, #thresholds, #timer, #isRunning, #reportCount, #fpsUnsubscribe}
 */
export class MetricsReporter {
	 ...
	 * @returns {Promise<void>} A promise that resolves when the reporter has started.
	 */
	start() {
		const orchestrator = this.#asyncOrchestrator;

		/* eslint-disable-next-line nodus/require-async-orchestration -- helper executed via orchestrator.run or called inline when orchestration is disabled */
		const runFn = async () => {
			if (this.#isRunning) {
			...
			// Start the underlying RenderMetrics service, which will emit its own events.
			// This is an async operation, so await it.
			await this.#renderMetrics.startTracking();

			// The orchestrator's plugins will handle metrics and alerts based on system events.
			// This service's only job is to manage the lifecycle.
		...
		// Policy: only use the orchestrator when the 'async' policy is enabled.
		const policies = this.#policiesManager;
		const asyncEnabled = policies?.getPolicy
			? policies.getPolicy("async", "enabled")
			: true;

		if (!asyncEnabled) {
			// Policy disables async orchestration for this environment — run inline.
			return runFn();
		}

		/* PERFORMANCE_BUDGET: 10ms */
		return orchestrator.run(runFn, {
			component: "MetricsReporter",
			operation: "start",
			// Add any relevant context for auditing or error handling
			// e.g., userId: this.#stateManager.getUserId(),
		});
	}

	/**
	 * Dispatches a metric update through ActionDispatcher when available, falling back to direct MetricsRegistry set.
	 * @private
	 */
	#dispatchMetric(key, value) {
		try {
			if (this.#stateManager?.managers?.actionDispatcher?.dispatch) {
		...
		try {
			// Prefer ActionDispatcher; if unavailable, do nothing (metrics will be collected by plugins when available)
			if (this.#stateManager?.managers?.actionDispatcher?.dispatch) {
				/* PERFORMANCE_BUDGET: 1ms */
				this.#stateManager.managers.actionDispatcher.dispatch(
					"metrics.set",
			...
			// If dispatcher isn't available (e.g., early bootstrap), we intentionally avoid direct writes here
		} catch {
			// swallow metrics errors - non-fatal
		}
	}
	...
	stop() {
		...
		return orchestrator.run(runFn, {
			component: "MetricsReporter",
			operation: "stop",
			// This is a synchronous operation, but orchestrating it ensures
			// consistent auditing and policy enforcement for start/stop actions.
		});
	}
	...
	#updateMetricsRegistry(renderMetrics) {
		this.#errorHelpers.try(
			() => {
				...
				this.#dispatchMetric("performance.fps", renderMetrics.fps);
				this.#dispatchMetric(
	 ...
	 * @private
	 * @param {object} renderMetrics - The latest metrics object from `RenderMetrics`. See `RenderMetrics.getCurrentMetrics()`.
	 * @returns {Array<object>} An array of detected alert objects.
	 */
			...
			() => {
				...
				const report = {
					...
					system: {
						...
						viewport: this.#uiBridge?.getScreenDimensions() || {
							width: 0,
							height: 0,
						},
	 ...
	 * @param {string} [operation="unknown"] - A more specific name for the operation (for context).
	 * @returns {void}
	 */
