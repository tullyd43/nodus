...
/* eslint-disable nodus/require-action-dispatcher, nodus/require-observability-compliance --
	...
	preserving enforcement elsewhere. */
...
export class MetricsRegistry {
	...
	increment(metricName, amount = 1, metadata = {}) {
		// Policy gate: consult shared policy adapter (fast sync path) before emitting metrics.
		try {
			const adapter = this.#stateManager?.managers?.policyAdapter;
			if (adapter?.shouldInstrumentSync) {
				const allowed = Boolean(
					adapter.shouldInstrumentSync({
						component: "metrics",
						operation: metricName || "increment",
			...
			// fail-open: ignore policy errors and proceed
		}
	...
	timer(metricName, duration, tags = {}) {
		// Policy gate: consult shared policy adapter (fast sync path) before emitting timing metrics.
		try {
			const adapter = this.#stateManager?.managers?.policyAdapter;
			if (adapter?.shouldInstrumentSync) {
				const allowed = Boolean(
	...
	histogram(metricName, value, tags = {}) {
		// Policy gate: consult shared policy adapter (fast sync path) before emitting histogram metrics.
		try {
			const adapter = this.#stateManager?.managers?.policyAdapter;
			if (adapter?.shouldInstrumentSync) {
				const allowed = Boolean(
					adapter.shouldInstrumentSync({
						component: "metrics",
						operation: metricName || "histogram",
						classification: "internal",
						tenantId: this.#stateManager?.currentTenant,
					})
				);
	...
	set(metricName, value, tags = {}) {
		// Policy gate: consult shared policy adapter (fast sync path) before emitting gauge metrics.
		try {
			const adapter = this.#stateManager?.managers?.policyAdapter;
			if (adapter?.shouldInstrumentSync) {
				const allowed = Boolean(
		...
		return (originalMethod) =>
			(...args) => {
				...
				try {
					...
					const duration = performance.now() - startTime;
					registry?.timer(metricName, duration, {
						...tags,
						error: true,
					});
					throw error;
