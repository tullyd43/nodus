...
export class BoundedStack {
	...
	constructor(maxSize = 50, { stateManager, ...options } = {}) {
		...
		this.#actionDispatcher =
			this.#stateManager?.managers?.actionDispatcher ?? null;
	...
	#handleEviction(evictedItem, reason) {
		...
		if (this.#actionDispatcher) {
			this.#actionDispatcher
				.dispatch("observability.stack_eviction", eventPayload)
				.catch((err) => {
					console.warn(
						`[BoundedStack] Failed to dispatch eviction event: ${err.message}`
					);
				});
		}
	}
