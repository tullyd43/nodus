...
export class ConditionRegistry {
  ...
  async evaluate(conditionDef, context, options = {}) {
    ...
    try {
      ...
      // Check cache first
      if (conditionDef.cacheable !== false && this.conditionCache.has(cacheKey)) {
        this.cacheHitCount++;
      ...
      console.error(`Condition evaluation error for ${conditionDef.type}:`, error);
  ...
  generateCacheKey(conditionDef, context) {
    ...
    try {
      ...
      return `condition_${JSON.stringify(keyData)}`.replace(/[^a-zA-Z0-9_]/g, '_');
    } catch (error) {
  ...
  hashRelevantContext(conditionDef, context) {
    ...
    const relevantKeys = conditionDef.contextKeys || ['userId', 'entityType', 'timestamp'];
  ...
  registerBuiltinConditions() {
    ...
    this.register('property_equals', (conditionDef, context) => {
    ...
    // Numeric comparison
    this.register('numeric_comparison', (conditionDef, context) => {
      const property = conditionDef.property;
    ...
    // Array membership
    this.register('in_array', (conditionDef, context) => {
      const property = conditionDef.property;
    ...
    this.register('regex_match', (conditionDef, context) => {
      const property = conditionDef.property;
    ...
    this.register('time_range', (conditionDef, context) => {
      const now = Date.now();
      const contextTime = context.timestamp || now;
      
      if (conditionDef.after && contextTime < conditionDef.after) return false;
      if (conditionDef.before && contextTime > conditionDef.before) return false;
    ...
    // Time of day
    this.register('time_of_day', (conditionDef, context) => {
      ...
      if (conditionDef.after) {
        const [afterHour, afterMinute] = conditionDef.after.split(':').map(Number);
        const afterInMinutes = afterHour * 60 + afterMinute;
        if (timeInMinutes < afterInMinutes) return false;
      }
      
      if (conditionDef.before) {
        const [beforeHour, beforeMinute] = conditionDef.before.split(':').map(Number);
    ...
    this.register('user_permission', (conditionDef, context) => {
      const userPermissions = context.userPermissions || context.user?.permissions || [];
    ...
    // User role check
    this.register('user_role', (conditionDef, context) => {
    ...
    // Entity type check
    this.register('entity_type', (conditionDef, context) => {
    ...
    // Rate limiting
    this.register('rate_limit', (conditionDef, context) => {
      ...
      const now = Date.now();
      const record = this.rateLimitStore.get(key) || { count: 0, windowStart: now };
    ...
    this.register('logical', async (conditionDef, context, options) => {
    ...
    this.register('javascript', (conditionDef, context) => {
      try {
  ...
  validateCondition(conditionDef) {
    const condition = this.conditions.get(conditionDef.type);
    if (!condition) {
      return { valid: false, error: `Unknown condition type: ${conditionDef.type}` };
    }
    ...
    for (const [property, rules] of Object.entries(schema)) {
      ...
      if (value !== undefined) {
        ...
        if (rules.enum && !rules.enum.includes(value)) {
          errors.push(`Property ${property} must be one of: ${rules.enum.join(', ')}`);
        }
        
        if (rules.pattern && typeof value === 'string') {
          const regex = new RegExp(rules.pattern);
          if (!regex.test(value)) {
            errors.push(`Property ${property} does not match pattern: ${rules.pattern}`);
          }
        }
      }
    }
  ...
  getStatistics() {
    return {
      ...
      cacheHitRate: this.evaluationCount > 0 ? this.cacheHitCount / this.evaluationCount : 0,
