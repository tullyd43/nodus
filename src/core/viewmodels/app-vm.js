...
class AppViewModel {
        ...
        if (this.listeners[event]) {
            this.listeners[event].forEach(callback => {
                try {
                    ...
                    console.error(`Error in AppViewModel ${event} listener:`, error);
                }
            });
        }
    }
    ...
    setupChildViewModelListeners() {
        ...
        this.itemViewModel.on('itemCreated', (item) => {
            this.showNotification(`Item "${item.name}" created successfully`, 'success');
        });
    ...
    setCurrentRoute(routeName) {
        const validRoutes = ['events', 'items', 'tags', 'collections', 'settings'];
