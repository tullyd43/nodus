...
class ItemViewModel {
        ...
        if (this.listeners[event]) {
            this.listeners[event].forEach(callback => {
                try {
                    ...
                    console.error(`Error in ItemViewModel ${event} listener:`, error);
                }
            });
        }
    }
    ...
    async deleteItem(itemId) {
        try {
            ...
            const items = this.state.items.filter(item => item.item_id !== itemId);
            
            this.setState({ 
                items,
                selectedItem: this.state.selectedItem?.item_id === itemId ? null : this.state.selectedItem
            });
    ...
    async logStockChange(itemId, oldQuantity, newQuantity, reason) {
        try {
            ...
            console.log(`Stock change: Item ${itemId}: ${oldQuantity} → ${newQuantity} (${reason})`);
        } catch (error) {
