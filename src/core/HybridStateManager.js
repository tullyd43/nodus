...
export class HybridStateManager {
    ...
    try {
      ...
      for (const entity of componentEntities) {
        try {
          ...
          this.adaptiveRenderer.registerComponent(
            entity.data.id,
            definition[0],
          );
        } catch (error) {
          console.error(
            "Failed to load component definition:",
            entity.data.id,
            error,
          );
        }
  ...
  async syncComponentDefinition(entity) {
    try {
      ...
      console.error(
        "Failed to sync component definition:",
        entity.data.id,
        error,
      );
    }
  ...
  renderFullMonitor(context) {
    ...
    const avgLatency =
      this.metrics.operationLatencies.length > 0
        ? (
            this.metrics.operationLatencies.reduce((sum, lat) => sum + lat, 0) /
  ...
  updateMetrics() {
    ...
    this.metrics.rendering.averageAdaptiveRenderTime =
