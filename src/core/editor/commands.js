...
/**
 * Insert code block - wraps selection or inserts empty block
 * Code blocks contain plain text only (no backticks as nodes)
...
export const insertCodeBlock =
	...
	(state, dispatch) => {
		...
		const codeBlock = state.schema.nodes.code_block.create(
			{ language },
			selectedText ? [state.schema.text(selectedText)] : []
		);
		...
		if (!state.selection.empty) {
			...
			// Insert code block after current block
			tr = state.tr.insert($from.after($from.depth), codeBlock);
		}
