...
export class EnhancedGridRenderer {
		...
		try {
			...
			if (this.#stateManager?.managers) {
				...
				this.#stateManager?.on?.("layoutRestored", (payload) => {
					...
					// V8.0 Parity: When a layout is restored via undo/redo, the renderer
					// needs to update its internal state to match.
					if (
						payload?.snapshot?.blocks &&
						(payload.direction === "undo" ||
							payload.direction === "redo")
					) {
						this.#applyLayoutSnapshot(payload.snapshot);
					}
				});
				this.#unsubscribeFunctions.push(() => {
					// hsm.off('layoutRestored', ...)
				});
			}
		} catch {
			/* noop */
	...
	/**
	 * Applies a layout snapshot by updating all block positions via the view model.
	 * This is used for history restoration (undo/redo).
	 * @private
	...
	#applyLayoutSnapshot(snapshot) {
		try {
			const vm = this.#appViewModel?.gridLayoutViewModel;
			if (!vm || typeof vm.updatePositions !== "function") return;

			const updates = (snapshot?.blocks || []).map((b) => {
				const pos = b.position || { x: b.x, y: b.y, w: b.w, h: b.h };
				return {
					blockId: b.blockId,
					x: pos.x,
					y: pos.y,
			...
			if (updates.length > 0) {
				vm.updatePositions(updates);
				this.refresh(); // Re-sync DOM with view model state
			}
		} catch (err) {
			this.#errorHelpers?.handleError(err, { component: "EnhancedGridRenderer", operation: "applyLayoutSnapshot" });
		}
	}
		...
		if (this.resizeItem) {
			...
			if (
				...
				if (this.resizeItem?.orig) {
					...
					// Cannot place and no orig snapshot — abort save
					this.#metrics?.increment("grid.resize_blocked");
					try {
						this.#toastManager?.warning(
	...
	// -------------------------
	/**
	 * Debug helper for unit tests — checks whether a block can be placed at a rect.
	 * @public
