...
export class LRUCache {
  ...
  set(key, value, ttlOverride = null) {
    ...
    try {
      ...
      if (this.options.auditOperations) {
        this.auditOperation('cache_set', { key: prefixedKey, size: itemSize, ttl, entityType: item.entityType });
      }
  ...
  getEntitiesWhere(predicate) {
    ...
    for (const [key, item] of this.cache.entries()) {
      if (!this.isExpired(key) && predicate(item.value, this.getUnprefixedKey(key))) {
        entities.set(this.getUnprefixedKey(key), item.value);
      }
    }
  ...
  getRecentlyAccessed(count = 10) {
    ...
    return new Map(items.map(([key, item]) => [this.getUnprefixedKey(key), item.value]));
  }
  ...
  delete(key) {
    ...
    if (this.options.auditOperations) {
      this.auditOperation('cache_delete', { key: prefixedKey, entityType: item.entityType });
    }
  ...
  updateEntityTypeMetrics(entityType, delta) {
    ...
    // Update total count
    this.metrics.totalEntitiesCached = Math.max(0, this.metrics.totalEntitiesCached + delta);
  }
  ...
  estimateValueSize(value) {
    ...
    if (Array.isArray(value)) {
      return value.reduce((sum, item) => sum + this.estimateValueSize(item), 0) + 100;
    }
  ...
  evictOldestItem() {
    ...
    if (this.options.onEvict) {
      this.options.onEvict(this.getUnprefixedKey(oldestKey), oldestItem.value, 'lru');
    }
    
    if (this.options.auditOperations) {
      this.auditOperation('cache_evict', { key: oldestKey, reason: 'lru', entityType: oldestItem.entityType });
    }
  }
  ...
  getMetrics() {
    ...
    const avgOperationTime = this.metrics.operationTimes.length > 0
      ? this.metrics.operationTimes.reduce((a, b) => a + b) / this.metrics.operationTimes.length
      : 0;

    // Calculate average entity size
    this.metrics.averageEntitySize = this.metrics.totalEntitiesCached > 0
    ...
    return {
      ...
      memoryUtilization: this.options.memoryLimit 
        ? Math.round((this.metrics.memoryUsage / this.options.memoryLimit) * 100)
        : null,
