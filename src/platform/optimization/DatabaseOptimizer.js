// NOTE: Do not import core classes (ForensicLogger) directly. Use the
// forensicLogger instance from the state manager (this.#forensicLogger) which
// is provided via `stateManager.managers.forensicLogger` to follow the
// `nodus/no-direct-core-instantiation` mandate.
...
export class DatabaseOptimizer {
	...
	async #runOrchestrated(operationName, operation, options = {}) {
		...
		if (!orchestrator) {
			// Fallback: execute directly when an orchestrator is not available
			return operation();
		}

		const policies = this.#stateManager?.managers?.policies;
		if (policies && !policies.getPolicy("async", "enabled")) {
			return null;
		}

		try {
			const runner = orchestrator.createRunner(
				`databaseOptimizer.${operationName}`
			);
			return await runner.run(
				() => this.#errorHelpers?.tryAsync(operation) || operation(),
				{
					label: `databaseOptimizer.${operationName}`,
					actorId:
						this.#stateManager?.userContext?.userId || "system",
			...
			this.#errorHelpers?.handleError?.(error, {
				...
				operation: `orchestrate.${operationName}`,
			});
			throw error;
		}
	}
	...
	/**
	 * Dispatches an action for observability, falling back to console if dispatcher is unavailable.
	 * @private
	 */
	#dispatchAction(actionType, payload) {
		const actionDispatcher = this.#stateManager?.managers?.actionDispatcher;
		const finalPayload = {
			...payload,
		...
		if (actionDispatcher) {
			actionDispatcher.dispatch(actionType, finalPayload);
		} else {
			console.warn(
				`[DatabaseOptimizer] ActionDispatcher not available. Action: ${actionType}`,
				finalPayload
			);
		}
	}
	...
	/**
	 * Emits a warning action for observability.
	 * @private
	 */
	...
	async initialize() {
		return this.#runOrchestrated("initialize", async () => {
			this.#dispatchAction("db_optimizer.initialize.start", {});
			...
			// Each of these steps is internally orchestrated, so we can await them directly.
...
á
	 ...
	 */
	async #verifySchema() {
		return this.#runOrchestrated("verifySchema", async () => {
	...
	#startAutoSuggestions() {
		...
		console.warn("💡 Auto-suggestions enabled");
	}
	...
	async #createIndexSuggestion(
		...
		// Use the injected forensicLogger instance when available instead of
		// importing core ForensicLogger directly. If the instance exposes
		// `createEnvelope`, call it and swallow any errors to avoid breaking
		// the suggestion creation flow.
		if (this.#forensicLogger?.createEnvelope) {
			Promise.resolve(
				this.#forensicLogger.createEnvelope({
					component: "DatabaseOptimizer",
					operation: "createIndexSuggestion",
					context: { tableName, jsonbPath, suggestionType },
				})
			).catch(() => {});
		}
		...
		try {
			this.#forensicLogger?.logAuditEvent(
				"DB_OPTIMIZATION_SUGGESTION_CREATED",
				{
					tableName,
					jsonbPath,
					suggestionType,
	...
	async rollbackOptimization(optimizationId) {
      ...
      `,
				...
				console.warn(
					`✅ Rolled back ${optimization.optimization_type}`
				);
	...
	async #generateOptimizationSuggestions() {
		...
		try {
			console.warn("🔍 Generating optimization suggestions...");
	...
	updateConfig(config) {
		// Use instance-based forensic envelope creation to avoid direct core
		// instantiation. Call createEnvelope on the injected forensicLogger if
		// available and swallow errors.
		if (this.#forensicLogger?.createEnvelope) {
			Promise.resolve(
				this.#forensicLogger.createEnvelope({
					component: "DatabaseOptimizer",
					...
					context: {
						providedKeys: config ? Object.keys(config) : [],
					},
				})
			).catch(() => {});
		}

		this.#forensicLogger?.logAuditEvent("DB_OPTIMIZER_CONFIG_UPDATED", {
		...
		console.warn("📝 DatabaseOptimizer configuration updated");
	}
	...
	async cleanup() {
		console.warn("[DatabaseOptimizer] Shutting down...");
		...
		console.warn("[DatabaseOptimizer] Cleanup complete.");
	}
}
