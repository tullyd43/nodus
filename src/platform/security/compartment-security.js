...
export default class CompartmentSecurity {
	...
	init() {
		...
		return this.#initializeStandardCompartments().then(() => {
			return this;
		});
	}
	 ...
	 * @memberof AutoGenerated
	 */
	checkAccess(requiredCompartments) {
		...
		// Sequentially check each required compartment.
		return requiredCompartments
			.reduce((promise, compartment) => {
				return promise.then((hasAccess) => {
					if (!hasAccess) return false; // Short-circuit if a check failed

					return this.#hasCompartmentAccess(
						compartment,
						userContext
					).then((result) => {
						if (!result) {
			...
			.then((allHaveAccess) => {
				if (!allHaveAccess) return false;

				// Check compartment combinations (some combinations may be restricted)
				if (
					!this.#validateCompartmentCombination(requiredCompartments)
				) {
	 ...
	 * @memberof AutoGenerated
	 */
	canAccessCompartment(compartment) {
		if (!this.#securityManager?.hasValidContext()) return false;
		const userContext = this.#securityManager.getSubject();

		return this.#hasCompartmentAccess(compartment, userContext);
	}
	 ...
	 */
	#hasCompartmentAccess(compartment, userContext) {
		if (!userContext) return Promise.resolve(false);
		...
		if (rules) {
			return this.#evaluateCompartmentRules(
				compartment,
				rules,
				userContext
			);
	 ...
	 */
	#evaluateCompartmentRules(compartment, rules, userContext) {
		...
		// Sequentially evaluate special conditions
		return rules.specialConditions.reduce((promise, condition) => {
			return promise.then((allPassed) => {
				if (!allPassed) return false;
				return this.#evaluateSpecialCondition(condition, userContext);
			});
		}, Promise.resolve(true));
	}
	 ...
	 */
	#evaluateSpecialCondition(condition, userContext) {
	 ...
	 */
	#initializeStandardCompartments() {
