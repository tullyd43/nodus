 ...
 *
 * @module RealtimeSync
 * @privateFields {#connection, #messageQueue, #subscriptions, #reconnectAttempts, #config, #stateManager, #pendingRequests, #heartbeatInterval, #metrics, #forensicLogger, #errorHelpers, #asyncService}
 */
export default class RealtimeSync {
	...
	constructor({ stateManager, options = {} }) {
		...
		if (!this.#asyncService) {
			throw new AppError(
				"AsyncOrchestrationService is required for RealtimeSync."
			);
		}

		this.#run = this.#asyncService.createRunner({
			labelPrefix: "storage.realtimeSync",
			actorId: "realtime.sync",
			eventType: "STORAGE_REALTIME_OPERATION",
			meta: { component: "RealtimeSync" },
		});
	}
	...
	init() {
		return this.#run(
			async () => {
				await this.#connect();
				this.#metrics?.increment("init.count");
				this.#audit("init", { config: this.#config });
				return this;
			},
			{
				labelSuffix: "init",
				eventType: "STORAGE_REALTIME_INIT",
				meta: {
	 ...
	 * TODO: Add JSDoc for method subscribe
	 ...
	 * TODO: Add JSDoc for method unsubscribe
	...
	push(options) {
		return this.#run(
			...
			{
				labelSuffix: "push",
				eventType: "STORAGE_REALTIME_PUSH",
				meta: {
	...
	pull(options) {
		return this.#run(
			...
			{
				labelSuffix: "pull",
				eventType: "STORAGE_REALTIME_PULL",
				meta: {
	...
	syncItem(item, operation) {
		return this.#run(
			() => this.#pushItem(item, operation),
			{
				labelSuffix: "syncItem",
				eventType: "STORAGE_REALTIME_SYNC_ITEM",
				meta: { itemId: item?.id ?? null, operation },
			}
		);
	}
	 ...
	 * TODO: Add JSDoc for method disconnect
