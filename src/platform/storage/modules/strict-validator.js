...
export default class StrictValidator {
	...
	constructor({ stateManager, options = {} }) {
		...
		// The orchestrator now handles metrics and forensic logging via plugins.
		this.#orchestrator = this.#stateManager?.managers?.asyncOrchestrator;
		this.#policyService = this.#stateManager?.services?.policy;
	 ...
	 */
	init() {
		// V8.0 Parity: All async operations must be orchestrated.
		return this.#orchestrator.run(
			async () => {
				console.log(
					"[StrictValidator] Strict validation rules initialized"
				);
				return this;
			},
			{ label: "StrictValidator.init", meta: { component: this.name } }
		);
	}
	...
	validate(entity, context = {}) {
		...
		return this.#orchestrator.run(
			async () => {
				// V8.0 Parity: Use environment-agnostic DateCore for timing.
			...
			{
				label: "StrictValidator.validate",
				meta: { entityId: entity?.id, entityType: entity?.entity_type },
			}
		);
	}
	...
	validateField(entityType, fieldName, value, context = {}) {
		...
		return this.#orchestrator.run(
			...
			{
				label: "StrictValidator.validateField",
	 ...
	 */
	#_validateSecurity(entity, context) {
		return this.#orchestrator.run(
			async () => {
				const errors = [];
				const warnings = [];

				const validClassifications = this.#policyService?.getPolicy(
					"system",
					"security_valid_classifications"
				) ?? [
					"public",
					"internal",
					"restricted",
				...
				if (!entity.classification) {
					...
					errors.push(
						`Invalid classification: ${entity.classification}`
					);
				}
				...
				if (
					...
					for (const compartment of entity.compartment_markings) {
						if (
							!StrictValidator.#_isValidCompartment(compartment)
						) {
							errors.push(
								`Invalid compartment marking: ${compartment}`
							);
						}
					}
				}
			...
			{
				label: "StrictValidator._validateSecurity",
				meta: { entityId: entity?.id },
			}
		);
	}

	#_validateCompliance(entity, context) {
		return this.#orchestrator.run(
			async () => {
				const errors = [];
				const warnings = [];
				...
				if (
					StrictValidator.#_containsHealthData(
						entity,
						this.#complianceRules
					)
				) {
			...
			{
				label: "StrictValidator._validateCompliance",
				meta: { entityId: entity?.id },
			}
		);
	}

	#_validateBusinessRules(entity, context) {
		return this.#orchestrator.run(
			async () => {
				const errors = [];
				const warnings = [];
				...
				if (
					entity.email &&
					!StrictValidator.#_isValidEmail(entity.email)
				) {
				...
				if (
					entity.phone &&
					!StrictValidator.#_isValidPhone(entity.phone)
				) {
				...
				if (entity.date_of_birth) {
					...
					if (age > 150) {
						warnings.push(
							"Age seems unusually high - please verify"
						);
					}
				}
			...
			{
				label: "StrictValidator._validateBusinessRules",
				meta: { entityId: entity?.id },
			}
		);
	}

	#_validateDataIntegrity(entity, context) {
		return this.#orchestrator.run(
			async () => {
				const errors = [];
				const warnings = [];
			...
			{
				label: "StrictValidator._validateDataIntegrity",
				meta: { entityId: entity?.id },
			}
		);
	}
	...
	/** @private @static */
	static #_calculateIntegrityHash(entity) {
		// This method is now wrapped by an orchestrator in its callers,
		// but for full compliance, it must also be wrapped as it contains async logic.
		// Since it's a static method, it must use the global orchestrator instance.
		const orchestrator = AsyncOrchestrator.getOrCreateGlobal();
		return orchestrator.run(
			...
			{
				label: "StrictValidator._calculateIntegrityHash",
				meta: { entityId: entity?.id, component: "StrictValidator" },
			}
		);
	}
