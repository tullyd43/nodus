...
export class EnhancedGridRenderer {
  ...
  enhanceExistingBlock(blockEl) {
    ...
    blockEl.style.gridColumnEnd = blockData.position.x + blockData.position.w + 1;
  ...
  addAccessibilityFeatures(blockEl, blockData) {
    ...
    blockEl.setAttribute('aria-describedby', `${blockData.blockId}-instructions`);
    ...
    if (!document.getElementById(`${blockData.blockId}-instructions`)) {
      ...
      instructions.textContent = 'Use arrow keys to move, Shift+arrow keys to resize, Enter to activate';
  ...
  moveBlock(blockData, deltaX, deltaY) {
    ...
    if (newX !== blockData.position.x || newY !== blockData.position.y) {
      ...
      const blockEl = this.container.querySelector(`[data-block-id="${blockData.blockId}"]`);
      if (blockEl) {
  ...
  getBlockDataFromElement(blockEl) {
    ...
    const currentLayout = this.appViewModel.gridLayoutViewModel.getCurrentLayout();
      ...
      if (now - lastFrameTime >= 1000) {
        ...
        if (!manualOverride) {
          ...
          if (fps < 30 && !this.performanceMode) {
            ...
            EventBus.emit('gridPerformanceMode', { fps, enabled: true, reason: 'auto' });
          } else if (fps > 50 && this.performanceMode) {
            this.disablePerformanceMode();
            EventBus.emit('gridPerformanceMode', { fps, enabled: false, reason: 'auto' });
          }
        }
  ...
  checkPerformancePolicyOverride() {
    ...
    try {
      ...
      const manualMode = this.appViewModel?.context?.getPolicy?.('system', 'grid_performance_mode');
  ...
  endEnhancedResize(e) {
    ...
    EventBus.emit('blockResizeEnd', { /* resize data */ });
    this.isResizing = false;
  }
