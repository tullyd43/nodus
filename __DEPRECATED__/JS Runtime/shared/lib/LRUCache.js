...
export class LRUCache {
	...
	/**
	 * Increment a metric in an orchestrated way so platform policies capture metadata.
	 * @private
	 */
	#incrementMetric(name, value = 1) {
		// Prefer ActionDispatcher so the platform's observability plugins capture metric updates.
		const dispatcher = this.#stateManager?.managers?.actionDispatcher;
		const metricKey = this.#options.keyPrefix
			? `cache.${this.#options.keyPrefix}.${name}`
			: `cache.${name}`;
		try {
			if (dispatcher?.dispatch) {
				/* PERFORMANCE_BUDGET: 1ms */
				dispatcher.dispatch("metrics.increment", {
					key: metricKey,
					value,
				});
				return;
		...
		// If dispatcher is unavailable we intentionally avoid direct metric writes here.
		// Platform instrumentation (MetricsReporter/plugins) will pick up metrics when available.
	}
	...
	get(key, securityContext = {}) {
		const execute = () => {
			...
			if (this.#options.enableMetrics) {
				this.#incrementMetric("hits"); // This is a 'hit'
			}
	...
	set(key, value, options = {}) {
		const execute = () => {
			...
			if (this.#cache.has(prefixedKey)) {
				...
				if (oldItem) {
					this.#incrementMetric("memory_bytes", -oldItem.size);
				}
			...
			// Add new item
			this.#cache.set(prefixedKey, item);
			this.#incrementMetric("memory_bytes", itemSize);
	...
	delete(key) {
		return this.#errorHelpers?.trace("cache.delete", () => {
			...
			this.#incrementMetric("memory_bytes", -item.size);
	...
	clear() {
		...
		const currentMemory =
			this.#getMetrics()?.get("memory_bytes")?.value || 0;
		if (currentMemory > 0) {
			this.#incrementMetric("memory_bytes", -currentMemory);
		}
		this.#incrementMetric("evictions", clearedCount);
		if (this.#options.auditOperations) {
	...
	invalidate(pattern) {
		...
		for (const key of toDelete) {
			...
			if (item) this.#incrementMetric("memory_bytes", -item.size);
	...
	#expireItem(key) {
		...
		this.#ttlMap.delete(key);
		this.#incrementMetric("memory_bytes", -item.size);
		this.#incrementMetric("expirations");
	...
	#evictOldestItem(reason = "lru") {
		...
		this.#incrementMetric("memory_bytes", -oldestItem.size);
