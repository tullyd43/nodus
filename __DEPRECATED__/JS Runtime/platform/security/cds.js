...
export const CDS = {
	...
	fetch(url, opts = {}) {
		...
		// No transport available — return a safe stub and log for visibility.
		console.warn(
	...
	fetchXHR(url, opts = {}) {
		...
		const transport =
			...
			globalThis.__NODUS_CDS_TRANSPORT__;
		if (typeof transport === "function") {
			return transport(validated.href, init).then((response) => {
				const textPromise =
					typeof response.text === "function"
						? response.text()
						: Promise.resolve(String(response?.responseText ?? ""));
...
export class CrossDomainSolution {
	 ...
	 * @memberof AutoGenerated
	 */
	automatedDowngrade({ logicalId, toLevel, sanitizerRules }) {
		// V8.0 Parity: Use error helper for robust execution.
		const tryAsync = this.#errorHelpers?.tryAsync;
		if (tryAsync) {
			return tryAsync(
				() => {
					return this.#executeAutomatedDowngrade({
						logicalId,
						toLevel,
						sanitizerRules,
					});
		...
		return this.#executeAutomatedDowngrade({
			...
			toLevel,
			sanitizerRules,
		}).catch(() => ({ approved: false, newEntityId: null }));
	}

	#executeAutomatedDowngrade({ logicalId, toLevel, sanitizerRules }) {
		// 1. Load the original, high-classification entity.
		return this.#stateManager
			.loadEntity(logicalId)
			.then((originalEntity) => {
				if (!originalEntity) {
					throw new Error(
	...
	#applySanitization(data, rules) {
		...
		console.warn("[CDS] Applying sanitization rules:", rules);
