# Nodus Enterprise v8.0.0-Beta - Final Hardening Checklist (Enhanced)
...
#### **Cryptography**
- [ ] **Post-Quantum Crates**: PQ algorithms via `pqcrypto`/ffi libs; `ring` used only for classical crypto
- [ ] **Key Separation**: Classical and post-quantum crypto implementations properly isolated
- [ ] **Crypto Agility**: Algorithm selection configurable without code changes
...
- [ ] **Async Lock Audit**: No `std::sync::RwLock` in async code paths; verified via grep/lint
- [ ] **Binary Serialization**: Hot-path telemetry uses binary encoding (postcard/rmp); no JSON in performance-critical paths
- [ ] **Atomic Metrics**: Lock-free atomics for high-frequency counters; snapshot on export
- [ ] **SQLite Tuning**: WAL mode, page cache, mmap, and prepared statements configured at startup
- [ ] **Memory Allocator**: Consider `jemalloc` vs system allocator for long-running services
- [ ] **Dead Code Elimination**: `cargo build --release` with LTO for optimized binaries
...
- [ ] **Reload Debounce**: Policy file watcher debounced to ≤ 1 Hz to avoid thrash
...
- [ ] **Audit Hook**: Every allow/deny emits audit event (ring-buffered, async flush)
...
- [ ] **Ring Buffer**: Audit writes buffered in in-memory ring buffer → async batch flush
...
- [ ] **Metrics Cardinality**: Bounded metric label growth to prevent unbounded memory usage
...
- [ ] **Graceful Shutdown**: SIGTERM handled with proper cleanup (30s timeout)
...
- [ ] **Temporal Indexes**: (object_id, ts) and (ts) indexed for append-only event tables
...
- [ ] **Execution Budgets**: Per-plugin CPU/memory/time budgets enforced (X ms / Y MB limits)
- [ ] **Schema Whitelist**: All plugin input/output deserialized via whitelisted schema types only
- [ ] **Plugin Isolation**: No direct plugin-to-plugin communication without explicit approval
...
This enhanced checklist incorporates world-class production systems engineering practices and represents the gold standard for enterprise-grade security platform releases. Each item should be verifiable with concrete evidence (test results, metrics, documentation, etc.).

## 🏅 Enhanced Items Summary

**New additions based on expert review:**
- **Async Lock Audit**: Prevents blocking async runtime
- **Binary Serialization**: Optimizes hot-path performance  
- **SQLite Tuning**: Production database optimization
- **Policy Debounce**: Prevents configuration thrash
- **Audit Ring Buffer**: High-performance audit logging
- **Plugin Execution Budgets**: Resource containment
- **Schema Whitelisting**: Supply chain security
- **Temporal Indexing**: Time-series query optimization
- **Post-Quantum Crypto**: Future-proof cryptography
- **Additional operational hardening items**
